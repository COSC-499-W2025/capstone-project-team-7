...
public struct WordokuGrid: Equatable {
    ...
    /// Main diagonal indices (top-left to bottom-right)
    public static func mainDiagonalIndices() -> [Int] {
        (0..<9).map { $0 * 9 + $0 }
    }
    
    /// Anti-diagonal indices (top-right to bottom-left)
    public static func antiDiagonalIndices() -> [Int] {
        (0..<9).map { $0 * 9 + (8 - $0) }
    }
}
