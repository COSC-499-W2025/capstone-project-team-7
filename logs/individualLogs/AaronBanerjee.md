...
**View Saved Projects Implementation [(PR #280)](https://github.com/COSC-499-W2025/capstone-project-team-7/pull/280)**: Converted the TUI "View Saved Projects" screen into a fully functional Electron/Next.js page with responsive design and comprehensive functionality. Created new /projects route with projects table displaying metadata including file counts, line counts, and language badges with action buttons (view details, delete, navigate). Implemented project detail modal showing overview statistics (files, lines, languages, total size) for quick project insights. Built complete API client in frontend/lib/api/projects.ts with getProjects(), getProjectById(), and deleteProject() methods for backend communication. Created reusable React components including ProjectsTable for the main list view and ProjectDetailModal for detailed project information display. Fixed critical backend data extraction bugs in projects_service.py by updating get_user_projects() to properly fetch and decrypt the scan_data field. Enhanced normalize_project_data() in project_routes.py to extract metadata from scan_data.summary when root fields are empty, supporting both old and new scan formats. Added authentication guards, loading states, error handling, and refresh functionality to the projects page. Implemented comprehensive test suite with 20 unit tests covering loading states, authentication, data display, modal interactions, delete functionality, error handling, and edge cases. Added date-fns dependency for relative timestamp display ("2 days ago") and @testing-library/dom with happy-dom for testing support. Successfully closed issue #263 with 1,523 additions and 83 deletions across frontend and backend.

**Code Analysis Enhancement [(PR #283)](https://github.com/COSC-499-W2025/capstone-project-team-7/pull/283)**: Significantly enhanced code_parser.py with eight new analysis categories providing actionable insights beyond basic metrics. Implemented dead code detection identifying unused functions, unreachable code blocks, and redundant imports with high/medium confidence levels. Added duplicate code detection finding similar blocks within files and across the codebase using hash-based comparison with configurable minimum block size. Built call graph analysis mapping function call relationships to understand code flow and dependencies. Created magic value detection flagging hardcoded numbers and strings that should be constants, with suggested constant names. Implemented error handling quality analysis detecting empty catch blocks and overly broad exception handling with severity levels (critical/warning). Added naming convention checking validating variable/function names follow language-specific conventions (snake_case, camelCase, PascalCase) with automated suggestions. Built nesting depth analysis identifying deeply nested code (>4 levels) that reduces readability with refactoring recommendations. Implemented data structure usage tracking for lists, dicts, sets, and other structures with usage examples. Added DirectoryResult.to_dict() method for proper JSON serialization supporting database storage of analysis results. Updated CodeAnalysisService.format_summary() to display all new analysis sections in human-readable format. Created comprehensive test script test_code_parser.py at project root with multiple output modes (--detailed, --json, --dead-code, --duplicates) for flexible analysis workflows. Verified support for 12+ languages including Python, JavaScript, TypeScript, TSX, Java, C, C++, Go, Rust, Ruby, PHP, and C#. Successfully closed issue #282 with 2,367 additions and 509 deletions, directly addressing peer evaluation feedback on code quality insights.

**Challenges & Learning:**
For PR #280, the main challenge was handling inconsistent data formats in scan_data - projects could have metadata in root fields or nested in scan_data.summary, and languages could be stored as arrays of objects, dictionaries, or arrays of strings. I resolved this by implementing flexible normalization logic that tries multiple extraction strategies, ensuring backward compatibility with older scans. The review feedback from Om highlighted areas for improvement including inconsistent auth token keys, console logs in production code, and the use of native confirm() dialogs. For PR #283, the primary challenge was balancing analysis depth with performance - Om noted that _detect_cross_file_duplicates() could cause memory issues on large codebases by storing all code blocks in memory. I learned about the trade-offs between comprehensive analysis and resource constraints, and the importance of configurable limits for production use. Additionally, working with tree-sitter for multi-language AST parsing required understanding language-specific node types while keeping analysis logic language-agnostic.

**Next Week Priorities:**
Next week, I'll implement the /projects/{id}/analysis page to display full project analysis results with detailed breakdowns of the new code quality insights. I'll add pagination and filtering/sorting to the projects table for better navigation with large project lists. I'll address the performance concerns in the code parser by implementing memory usage limits and batch processing for cross-file duplicate detection. I'll also clean up the code by removing console logs, standardizing auth token keys across the codebase, and replacing native dialogs with custom modals for consistent UX. Additionally, I'll explore implementing a "Scan New Project" button directly in the UI rather than relying on external scripts.


Impact:

This week's work significantly improves both the frontend user experience and backend analysis capabilities of the portfolio application. The View Saved Projects feature provides users with an intuitive way to browse, manage, and navigate their project portfolio through a modern web interface rather than a text-based TUI. The code analysis enhancements directly address feedback from peer evaluations by providing actionable insights that help developers understand code quality beyond basic metrics - identifying technical debt (dead code, magic values), spotting potential bugs (error handling issues), and suggesting improvements (naming conventions, nesting depth). Together, these improvements make the portfolio application more accessible to users and more valuable as a code quality tool, supporting the project's goal of helping developers showcase and improve their work.

Issues Resolved:
[#263](https://github.com/COSC-499-W2025/capstone-project-team-7/issues/263)
[#282](https://github.com/COSC-499-W2025/capstone-project-team-7/issues/282)

PR's:
[(PR #280)](https://github.com/COSC-499-W2025/capstone-project-team-7/pull/280)
[(PR #283)](https://github.com/COSC-499-W2025/capstone-project-team-7/pull/283)
<img width="1311" height="156" alt="image" src="https://github.com/user-attachments/assets/d67362be-95bd-401e-b8a9-42081cea0dae" />
<img width="1259" height="157" alt="image" src="https://github.com/user-attachments/assets/3511cf9f-be2f-4d8e-a1d6-47a0ec8767ca" />
<img width="1055" height="613" alt="image" src="https://github.com/user-attachments/assets/11ea9ef2-ca33-4cb1-ba2f-fbf83f081db3" />
...
Issues resolved include:[#199](https://github.com/COSC-499-W2025/capstone-project-team-7/issues/199),[#219](https://github.com/COSC-499-W2025/capstone-project-team-7/issues/219),[#222](https://github.com/COSC-499-W2025/capstone-project-team-7/issues/222)
