...
Implemented incremental portfolio refresh functionality ([PR #242](https://github.com/COSC-499-W2025/capstone-project-team-7/pull/242)) with SHA-256 hash-based deduplication, enabling efficient detection of duplicate files across user projects and preventing unnecessary database writes.
Built the append-upload API endpoint that intelligently merges new uploads into existing projects by skipping duplicates, updating changed files, and adding new ones—improving upload efficiency and data consistency.
Developed TUI client methods in PortfolioRefreshAPIService to integrate the new refresh and append functionality into the existing CLI interface, maintaining consistency with our API-first architecture.
Achieved comprehensive test coverage with 13 tests for the new endpoints, ensuring reliability and catching edge cases in the deduplication logic.
