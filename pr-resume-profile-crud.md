...
- `list_resumes()` preserves server-side pagination (`count="exact"` + `.range()`) and excludes profile records using `.neq("name", PROFILE_RECORD_NAME)` so no Python-side filtering is needed

**Frontend:**
- Added `getResumeProfile()` and `saveResumeProfile()` API client functions in `user-resume.ts`
- Created `ProfileForm` component with editable sections for Contact, Education, Experience, Skills, and Awards (add/remove/update)
...
- Fixed auto-save in editor to always include `structured_data` and `is_latex_mode` in save payload
...
> Tested manually by saving profile data and verifying it persists across page reloads.
