...
import { drive, ocr, parser } from '../services/index.js';
...
function pickReceiptDateSlug(rawDate?: string | null) {
	if (!rawDate) return null;
	const parsed = new Date(rawDate);
	if (Number.isNaN(parsed.getTime())) return null;
	return formatDateForSlug(parsed);
}
...
function buildReceiptFilename(
	...
	const parts: string[] = [];
	const dateSlug = pickReceiptDateSlug(parsed?.date) || formatDateForSlug(fallbackDate);
	if (dateSlug) parts.push(dateSlug);
	const vendorSlug = slugify(parsed?.vendor);
	if (vendorSlug) parts.push(vendorSlug);
	const totalSlug = slugify(
		parsed?.total != null ? String(parsed.total).replace(/[^0-9.]+/g, '') : null,
	);
	if (totalSlug) parts.push(`total-${totalSlug}`);

	const base = parts.length ? parts.join('_') : jobId;
...
export async function processReceipt(job: Job) {
	...
	try {
		...
		const extension = resolveFileExtension(filePath, job.payload?.mediaContentType);
		const filename = buildReceiptFilename(
			{ ...parsed, date: finalDate ?? undefined },
			job.id,
			extension,
			processedAt,
		);
		const monthFolder = `${processedAt.getFullYear()}-${pad(processedAt.getMonth() + 1)}`;
		const uploaded = await drive.upload(buffer, filename, { subfolderName: monthFolder });
		logger.info('Uploaded to drive', { ...uploaded, filename });

		// send a reply back to the user who sent the receipt
		if (job.payload && job.payload.from) {
			const spreadsheetId = env.SPREADSHEET_ID || process.env.SPREADSHEET_ID;
			if (!spreadsheetId) {
				throw new Error('Missing SPREADSHEET_ID env');
			}
			const to = job.payload.from;
			const baseFolderId = env.DRIVE_UPLOAD_FOLDER_ID || process.env.DRIVE_UPLOAD_FOLDER_ID;
			const folderId = uploaded.folderId ?? baseFolderId;
			const folderUrl = folderId ? `https://drive.google.com/drive/folders/${folderId}` : undefined;
			const session: Session = storeReceiptData(
				to,
				{
					company: companyName,
		...
		if (to) {
			const latest = getSession(to);
			if (latest?.lastErrorJobId === job.id) {
