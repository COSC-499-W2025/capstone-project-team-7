...
class ScanConfigScreen(ModalScreen[None]):
    ...
    def on_mount(self, event: Mount) -> None:  # pragma: no cover
    ...
    def on_key(self, event: Key) -> None:  # pragma: no cover
        if event.key == "escape":
            self.app.post_message(ScanCancelled())
            self.dismiss(None)
...
class AIKeyScreen(ModalScreen[None]):
    ...
    def compose(self) -> ComposeResult:
        yield Vertical(
            ...
            Static("Your OpenAI key and settings stay in-memory for this session only.", classes="dialog-subtitle"),
            Input(value=self._default_key, placeholder="sk-...", password=True, id="ai-key-input"),
            Horizontal(
    ...
    def _submit(self) -> None:
        ...
        temp_str = temp_input.value.strip()
        if temp_str:
            try:
                parsed = float(temp_str)
                if 0.0 <= parsed <= 2.0:
                    temperature = parsed
                else:
                    raise ValueError
                ...
                return
        tokens_str = tokens_input.value.strip()
        if tokens_str:
            try:
                parsed_tokens = int(tokens_str)
                if parsed_tokens > 0:
...
class ConsentScreen(ModalScreen[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Vertical(
            ...
            Horizontal(Button("Close", id="consent-close")),
...
class PreferencesScreen(ModalScreen[None]):
    ...
    def _apply_general_settings(self) -> None:
        max_size_input = self.query_one("#pref-max-size", Input)
        follow_checkbox = self.query_one("#pref-follow-symlinks", Checkbox)
        max_size = self._summary.get("max_file_size_mb")
        max_size_input.value = str(max_size) if max_size is not None else ""
    ...
    def _collect_profile_inputs(self) -> Optional[Dict[str, Any]]:
        ...
        if not name:
            self._set_message("Enter a profile name before saving.", tone="error")
            return None
        exts_input = self.query_one("#pref-extensions", Input)
        ...
        desc_input = self.query_one("#pref-description", Input)
        description = desc_input.value.strip() or "Custom profile"
        return {
            "name": name,
    ...
    def _collect_settings(self) -> Optional[Dict[str, Any]]:
        max_size_input = self.query_one("#pref-max-size", Input)
        follow_checkbox = self.query_one("#pref-follow-symlinks", Checkbox)
        max_size_str = max_size_input.value.strip()
        payload: Dict[str, Any] = {}
        if max_size_str:
            try:
                payload["max_file_size_mb"] = int(max_size_str)
            except ValueError:
                self._set_message("Max file size must be an integer.", tone="error")
                return None
        payload["follow_symlinks"] = bool(follow_checkbox.value)
        return payload
...
class ScanResultsScreen(ModalScreen[None]):
    ...
    def compose(self) -> ComposeResult:
        button_widgets = [Button(label, id=f"scan-action-{action}") for action, label in self._actions]
        actions_layout = (
        ...
        if TextLog:
            log_widget = TextLog(highlight=False, markup=True, wrap=True, id="scan-results-log")
        else:
        ...
        log_panel = Vertical(
            context_label,
            Vertical(log_widget, file_list, id="scan-results-output", classes="scan-results-output"),
        ...
        yield Vertical(
            ...
            Horizontal(log_panel, actions_panel, classes="scan-results-body"),
            classes="dialog scan-results-dialog",
        )

    def on_mount(self, _: Mount) -> None:  # pragma: no cover
    ...
    def display_output(self, text: str, *, context: Optional[str] = None, allow_horizontal: bool = False) -> None:
        if context:
